...
# type your code for Question 2 here, and Knitrdu
ggplot(data=sfo_feb_flights, aes(x=arr_delay)) + geom_histogram()

# Median arrival delay
sfo_feb_flights %>% summarize(median_ad = median(arr_delay))
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(mean_ad=mean(arr_delay), IQR=IQR(arr_delay), n=n())
...
#Answer C. Given the daa is skewed with several outliers, the mean would be skewed with these outliers and would therefore me more relaible a measure to determine which months to avoid. 
...
nycflights %>% mutate(avg_speed = air_time/distance) %>% select(tailnum, avg_speed) %>% arrange(desc(avg_speed))
...
nycflights <- nycflights %>% mutate(avg_speed = air_time/distance)
ggplot(data=nycflights, aes(x=distance, y=avg_speed)) +geom_point()
ggplot(data=nycflights, aes(x = distance)) +geom_histogram()
